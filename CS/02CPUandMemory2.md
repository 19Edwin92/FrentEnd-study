...
- 비선점 스케쥴링은 시분할 시스템에서 주로 사용되며, 먼저 온 순서대로 실행하거나 우선 순위에 따라 실행하는 알고리즘이 적용될 수 있다. 
  ...
  |FCFS, First Come , First Serve|먼저 도착한 작업이 먼저 실행되는 방식|
  |SJF, Shorted Job First|실행 시간이 가장 짧은 작업을 우선으로 실행하는 방식|
  |HRN, Highest Response-ratio Next|응답시간과 작업대기 시간을 고려하여 작업의 우선순위를 결정하는 알고리즘으로, 각 작업에 대해 응답 시간 대기 시간을 비율로 계산하여 우선 순위로 사용한다. 즉 HRN은 작업의 응답시간이 짧고 대기 시간이 적은 작업에 높은 우선순위를 부여하여 실행한다. 응답시간을 고려하기에 응답성이 개선되지만, 계산 비용이 높고, 작업 대기 시간이나 서비스 시간이 긴 작업에 분리|
...
CPU의 MMU(Memory Management Unit)는 가상 주소와 물리 주소 간의 변환을 담당하는 유닛으로, 가상 주소를 사용하여 캐시의 인덱스로 사용되는 태그를 생성하고, 가상 주소를 캐시에 액세스 하여, 캐시 히트 또는 캐시 미스 여부를 판단한다. 

- 캐시히트 : MMU 캐시로부터 데이터를 가져온다. 
- 캐시미스 : MMU 메인 메모리로부터 데이터를 가져와 캐시에 업데이트 한다. 
- 가상주소 : 프로세스가 사용하는 주소 공간
- 물리주소 : 실제 하드웨어 메모리 주소 공간 

먼저 가상공간(가상 메모리)는 프로세스가 생성하는 것으로 실제 물리 메모리의 크기에 제한받이 않는다. 각 프로세스는 독립된 가상 주소 공간을 가지며, 다른 프로세스의 메모리에 접근할 수 없다. MMU는 가상 주소를 물리 주소로 변환하여 올바른 메모리 위치에 액세스하는 것을 보장한다. 

<br/>

**레지스터와 캐시**<br/>
레지스터는 CPU 내부에 있으며, CPU와 매우 밀접한 관련이 있기 때문에 일반적으로 캐시로 분류되지 않는다. 반면에 SRAM의 L1~L3 는 메모리 계층 구조에서 CPU와 메인 메모리 사이의 중간 저장소로 작용하여 액세스 지연 시간을 최소화 하고 전체적인 시스템 성능을 향상시킨다. SRAM은 주로 데이터 및 명령어를 캐시하여 액세스 속도롤 향산시킨다. 반면에 레지스터는 CPU가 직접 액세스하는 고속 메모리로, 명령어를 실행하거니 데이터를 실행하는데 처리되는 작용 용량의 저장 공간일 뿐이다. 

<br/>

**캐시의 지역성**<br/>
캐시의 지역성은 컴퓨터 프로그램이 특정 시간 동안 일부 메모리 영역에 집중되는 경향을 의미한다. 
1. 시간적 지역성 : 최근에 액세스한 메모리 위치에 다시 액세스할 가능성이 높음을 의미
2. 공간적 지역성 : 최근에 액세스한 메모리 위치의 인접한 위치에 다시 액세스할 가능성이 높음을 의미
3. 정리하면, 캐시는 이러한 지역성을 이용하여 자주 액세스되는 데이터와 명령어를 캐시에 보관함으로 액세스 시간을 줄인다. 

<br/>

**캐시매핑**<br/>
캐시 매핑은 메모리 주소를 캐시 라인에 매핑하는 방법을 의미하며, 주요한 매핑으로는 직접, 연관, 집한 연관 매핑이 있다. 

- 캐시라인 : 캐시 메모리의 기본 저장 단위, 캐시는 주로 블록 또는 라인이라고 불리는 고정 크기의 데이터 단위로 구성되어 있다. 이 블록 또는 라인은 메인 메모리에서 가져온 데이터와 해당 데이터의 주소 정보를 포함한다. 캐시 라인은 일반적으로 여러 개의 연속된 메모리 위치를 저장하는데, 이는 공간적 지역성을 활용하기 위한 것이다. 

1. 직접매핑 : 메모리 주소를 정확히 하나의 캐시 라인에 매핑하는 방식, 메모리 주소의 일부 비트를 캐시의 인덱스로 사용, 주소 충돌이 발생될 수 있지만, 매핑이 간단하고 빠른 액세스 속도를 제공하는 장점이 있다. 
  - 주소 충돌 : 서로 다른 메모리 블록의 주소가 동일한 인덱스를 가지는 경우 등
2. 연관매핑 : 메모리 주소를 캐시의 모든 라인에 대해 비교하는 방식으로, 메모리 주소 전체가 캐시 태그로 사용되며, 태그 비교를 통해서 캐시를 탐색, 이때 주소 충돌은 발생되지 않지만, 모든 라인을 비교한다는 점에서 검색 시간이 오래 발생된다. 
3. 집합 연관 매핑 : 메모리 주소를 여러 개의 집합으로 분할하고, 각 집합 내에서 연관 매핑을 수행하는 방식이다. 메모리 주소 일부 비트를 캐시의 인덱스로 사용하기에, 연관 매핑보다 적은 검색 시간과 더 나은 캐시 이용률을 제공하는 중간적인 방식이다.    

<br/>

**메인메모리(주기억장치)와 메모리 할당**<br/>
메인 메모리는 프로세스에게 필요항 메모리 공간을 할당하는데, 메인 메모리의 일부 영역을 프로세스에게 주어 프로세스가 이를 사용할 수 있도록 하는 것을 말한다. 프로세스는 실행을 위해서 코드, 데이터, 스택 등의 메모리 공간이 필요한데 이를 메인 메모리에서 할당받는다. 할당된 메모리는 레지스터와 캐시와는 별도의 메모리 공간이다. 

- 메인메모리 : 프로그램 코드, 데이터, 스택 등이 저장되며, CPU와 상대적으로 느린 속도로 데이터에 액세스 한다. 
- 레지스터 : CPU 내부에서 매우 빠른 속도로 데이터에 액세스 할 수 있는 작은 용량의 저장공간, 명령어와 데이터를 저장하고 처리
- 캐시메모리 " CPU와 메인 메모리 사이의 고속임시저장소, 주로 데이터와 명령어를 저장하여, CPU의 액세스 지연 시간을 줄인다. 

1. 연속할당
  - 메인 메모리에서 연속적인 메모리 공간을 할당받는 방식을 말한다. 연속적인 주소 범위를 가지며, 고정분할, 가변 분할, 선점 불할 할당 등이 있다. 

      |종류|설명|
      |:--|:--|
      |고정분해 할당|메인 메모리를 고정된 크기의 파티션으로 나누고, 각 파티션을 프로세스에게 할당하는 방식, 단점으로는 메모리 내에 프로세스 크기보다 큰 공간이 낭비될 수 있으며 이를 내부 단편화라고 부른다.|
      |가변분할 할당|동적으로 메모리를 분할하여, 프로세스에게 필요한 크기만큼 할당한다. 메모리를 효율적으로 사용할 수 있지만, 메모리보다 큰 프로세스는 할당받지 못한다는 점에서 외부 단편화 및 할당과 해제에 따른 메모리 관리 부담이 발생된다.|
      |선점분할할당|프로세스의 실행 중에 다른 프로세스가 할당된 메모리 영역을 선점할 수 있도록 하는 것이다. 우선순위나 시간 할당량에 따라 프로세스가 메모리를 선점하게 된다.|

2. 불연속할당
  - 연속되지 않는 메모리 공간을 할당받는 방식으로, 메인 메모리에서 분산된 주소 공간을 할당 받는다, 페이징, 세그먼테이션, 가상 메모리 할당 등이 있다. 

      |종류|설명|
      |:--|:--|
      |페이징|프로세스와 메인 메모리를 고정된 크기의 블록으로 분할하는 메모리 관리방식으로, 메인메모리와 프로세스 간의 주소 변환을 통해 페이지 단위로 데이터를 관리한다. 페이징은 내부 단편화가 없으며, 외부 단편화의 문제를 해결한다.|
      |세그먼테이션|프로세스를 서로 다른 크기의 세그먼트로 분할하는 메모리 관리 방식이다. 세그먼트는 프로그램의 논리적인 단위로, 코드, 데이터, 스택 등으로 구성된다. 프로세스는 가상 주소 공간을 세그먼트로 분할하고, 해당 세그먼트를 메인 메모리의 프레임에 매핑하여 사용한다. |
      |페이지드 세그면테이션|페이징과 세그먼테이션을 혼합한 메모리 관리 기법이로, 프로세스의 가상 공간을 세그먼트로 분할하고, 각 세그먼트를 페이지로 나누어 메모리에 로드하는 방식을 동작한다.|

  - 페이지드 세그먼테이션
    - 오프라인 페이지 교체 : 프로세스가 실행되기 전에 메모리 사용 패턴을 분석하여 페이지 교체 정책을 결정하는 방식으로, 가장 효율적인 페이지 교체 정책을 설정한다. 그러나 실제 시스템에 적용하기 어렵고, 예측의 정확성이나 대응력에 제약이 있다. 
    - 시간기반 페이지 교체 : 페이지가 얼마나 오래 사용되지 않았는지를 기준으로 페이지를 교체하는 방식이다. 최근에 사용되지 않은 페이지를 교체하여 페이지 부재를 최소화하는 데 중점을 둔다. 
    - 빈도기반 페이지 교체 : 페이지의 사용 빈도를 기준으로 페이지를 교체하는 방식이다. 가장 적게 사용된 페이지를 교체 대상으로 선택하여 메모리에 자주 액세스되는 페이지를 유지하는 것이 목표이다. 
    - 정리하면, 페이지 교체 알고리즘은 메모리의 한계적인 크기로 인해 페이지 부재가 발생하는 상황에서 효율적인 메모리 관리를 위해 사용된다. 
